...
// PROMISE — это объект, который представляет собой результат асинхронной операции: либо успешный, либо ошибку. Проще говоря, это обещание, что код когда-нибудь выполнится и выдаст результат. 
// Данная технология нужна для работы с асинхронными задачами — например, загрузкой данных с сервера, чтением файлов или чем-то ещё, что занимает время. Пока промис в ожидании, остальной код спокойно выполняется дальше и не зависает

// callBackHell - это нечитаемый код, который представляет собой последовательность асинхронных операций в виде большого количества функций обратного вызова
...
setTimeout(() => {
    ...
    setTimeout(() => {
        // добавление статуса для единицы товара
        product.status = 'was ordered';
        setTimeout(() => {
            console.log(product);
        }, 2000)
    }, 2000);
...
// Чтобы не попасть в callBackHell, необходимо использовать promise:
console.log('оформление заказа');
...
// resolve - это аргуметн, обозначающий функцию, вызываемую при выполнении обещания.
// reject - это аргумент, обозначающий функцию, вызываемую при НЕвыполнении обещания, например, если мы ссылаемся на несуществующий сервер или не получили данные от него.

// .then() - это метод обработки положительного результата промиса, который позволяет вести код последовательной и читаемой цепочкой асинхронных операций.
// .catch() - это метод обработки отрицательного результата промиса, который позволяет миновать весь чейнинг перед ним и обработать ошибку.
// .finally() - это метод, который выполнит переданную ему функцию независимо от результата работы промиса.

.catch(() => {
    console.error('возникла ошибка');
}).finally(() => {
    console.log('операция завершина, очистка формы');
})

// Методы .all() и .race()
// I -- Promise.all([pr1, pr2, pr3, ...]) - это метод, принимающий в себя массив с промисами, который также можно обработать при помощи .then(), который выполнится только когда все промисы в массиве будут выполнены.
const fakeRequest = time => {
    return new Promise(resolve => {
        setTimeout(() => resolve(), time)
    });
}
fakeRequest(2000).then(() => console.log('2000 ms'));
fakeRequest(1000).then(() => console.log('1000 ms'));

Promise.all([fakeRequest(1000), fakeRequest(2000)]).then(() => console.log('все запросы выполнились'));

// II -- Promise.race([pr1, pr2, pr3, ...]) - это метод, который также принимает в себя массив с промисами и выполняется сразу при выполнении промиса, сработавшего быстрее всех в массиве
Promise.race([fakeRequest(1000), fakeRequest(2000)]).then(() => console.log('один из запросов выполнился'));

// API и FETCH()
// API (Application Programming Interface) — это программный интерфейс, который позволяет различным программам взаимодействовать друг с другом. API определяет набор правил и инструментов, позволяющих одной программе получать доступ к функциям или данным другой программы.
// Примером такого программного интерфейса может быть DOM API, который включает в себя методы и инструменты для взаимодействия со страницей.
// Fetch API в JavaScript — это встроенный интерфейс для выполнения HTTP-запросов, который предоставляет готовые решения для получения ресурсов (например, данных) через сеть. В отличие от старого XMLHttpRequest, Fetch API работает на основе промисов, что упрощает асинхронный код.
// Для работы с Fetch API используется метод fetch()

// I -- создание GET-запроса и обработка полученных данных:
fetch('https://jsonplaceholder.typicode.com/todos/1')
    // Так как fetch() возвращает нам промис, мы можем обрабатывать его через цепочку .then() 
    .then(response => {
        // С помощью метода .json(), мы можем трансформировать результат работы fetch() из формата JSON в JS-формат, но особенность данного метода состоит в том, что он возвращает промис. Это сделано для дальнейшего чейнинга
        return response.json()
    })
    // Как только наши данные будут трансформированны, мы выведем их в консоль
    .then(json => console.log(json));

// II -- создание POST-запроса и обработка полученных данных:
fetch('https://jsonplaceholder.typicode.com/posts',
    // В качестве второго аргумента fetch() принимает объект с "настройками"
    {
        // метод (тип) запроса:
        method: "POST",
        // данные, отправляемые через API
        body: JSON.stringify({title: "Our title"}),
        // заголовки, которые дают серверу понять, с какими данными тому придётся работать
        headers: {
...
// TRY - CATCH 
// Во избежании блокировки всего кода из-за ошибки на определённой строке, используется следующая конструкция:
try {
    ...
    console.log('error');
    // В блоке catch, мы можем обрабатывать объект ошибки
    // console.log(e);
    // Для работы с ошибками существуют свойства: .name, .message, .stack
    // console.log(e.name);
    // console.log(e.message);
    // console.log(e.stack);
}
console.log('still code execution');
// С этой конструкцией также можно использовать .finally